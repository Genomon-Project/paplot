...
function init() {
    ...
    // draw integral bar
    {
        ...
        div_legend.items =  sig_data.signatures;
        div_legend.colors = sig_data.sig_colors;
        ...
        var dataset = sig_data.get_bars_data();
...
function draw_signature(name, signature_id, change_id) {
    ...
    svg_al.append("svg")
        ...
        .attr({
            ...
            'transform': 'translate(' + SIGNATURE_AXIS_YW + ',' + 12 + ')',
        });
    ...
    for (var c in change_list)  {
        ...
        // title
        if (c == 0) {
            bar.options.titles[0] = new bar.title_template(style_signature.title_signature_y);
