...
if (sig_data.mutations.length > 0) {
    div_integral = new mut_bar("div_integral");
...
function update_div() {
    ...
    var legend_w = 0;
    if (div_integral != 0) {
        legend_w = Number(d3.select("#div_rate_legend_svg").select("svg").style("width").replace("px", ""));
    }
...
sig_draw.init = function () {
    ...
    if (div_integral != 0) {
        // draw rate bar
        {
            ...
            var dataset_rate = sig_data.get_bars_data(true);
            ...
            for (var r = 0; r < dataset_rate.data.length; r++) {
                ...
                div_rate.dataset[r].color_fill = sig_data.sig_colors[r];
        ...
        // draw integral bar
        {
            ...
            var dataset_num = sig_data.get_bars_data(false);
            ...
            for (var n = 0; n < dataset_num.data.length; n++) {
                ...
                div_integral.dataset[n].color_fill = sig_data.sig_colors[n];
...
function draw_signature(name, signature_id, change_id) {
    ...
    var change = [];
    var change_colors = [];
    for (var s1 in sig_data.substitution)  {
        change.push(sig_data.substitution[s1].name);
        change_colors.push(sig_data.substitution[s1].color);
    }
        ...
        svg_sub.append("g")
            ...
            .on("mouseover", function(d, i) {
                ...
                for (var p=0; p < bar_dataset.tooltip[d][i].length; p++) {
                    d3.select("#tooltip").append("p").attr("id", "text").append("pre").text(bar_dataset.tooltip[d][i][p]);
                }
